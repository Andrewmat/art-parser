...
Parser.of = Parser['fantasy-land/of']; //           getData :: Parser e a s
...
exports.fail = fail;
const succeedWith = Parser.of; //           either :: Parser e a s -> Parser e (Either e a) s
...
const char = function char(c) {
  ...
  return new Parser(function char$state(state) {
    ...
    if (index < target.length) {
      return target[index] === c ? updateParserState(state, c, index + 1) : updateError(state, `ParseError (position ${index}): Expecting character '${c}', got '${target[index]}'`);
    }
...
const str = function str(s) {
  ...
  return new Parser(function str$state(state) {
    ...
    if (rest.length >= 1) {
      return rest.startsWith(s) ? updateParserState(state, s, index + s.length) : updateError(state, `ParseError (position ${index}): Expecting string '${s}', got '${rest.slice(0, s.length)}...'`);
    }
...
const regex = function regex(re) {
  ...
  return new Parser(function regex$state(state) {
    ...
    if (rest.length >= 1) {
      ...
      return match ? updateParserState(state, match[0], index + match[0].length) : updateError(state, `ParseError (position ${index}): Expecting string matching '${re}', got '${rest.slice(0, 5)}...'`);
    }
...
const digit = new Parser(function digit$state(state) {
  ...
  if (target.length > index) {
    return target.length && target[index] && /[0-9]/.test(target[index]) ? updateParserState(state, target[index], index + 1) : updateError(state, `ParseError (position ${index}): Expecting digit, got '${target[index]}'`);
  }
...
exports.digit = digit;
const digits = many1(digit).map(x => x.join('')).errorMap((_, index) => `ParseError (position ${index}): Expecting digits`); //           letter :: Parser e Char s
...
const letter = new Parser(function letter$state(state) {
  ...
  if (target.length > index) {
    return target.length && target[index] && /[a-zA-Z]/.test(target[index]) ? updateParserState(state, target[index], index + 1) : updateError(state, `ParseError (position ${index}): Expecting letter, got '${target[index]}'`);
  }
...
const letters = many1(letter).map(x => x.join('')).errorMap((_, index) => `ParseError (position ${index}): Expecting letters`); //           anyOfString :: String -> Parser e Char s
...
const anyOfString = function anyOfString(s) {
  return new Parser(function anyOfString$state(state) {
    ...
    if (target.length > index) {
      return s.includes(target[index]) ? updateParserState(state, target[index], index + 1) : updateError(state, `ParseError (position ${index}): Expecting any of the string "${s}", got ${target[index]}`);
    }
...
const whitespace = many1(anyOfString(' \n\t\r')).map(x => x.join('')); //           optionalWhitespace :: Parser e String s

exports.whitespace = whitespace;
const optionalWhitespace = many(anyOfString(' \n\t\r')).map(x => x.join('')); //           recursiveParser :: (() => Parser e a s) -> Parser e a s
